...
When('"User A" create column {int} row {int} a new table', { timeout: STEP_TIMEOUT }, async function (column, row) {
  withErrorHandling(() => createTable(pageA, column, row), "error occured while creating table");
  tableColumnNum = column;
  tableRowNum = row;
});

When('"User A" write {string} at the row {int} column {int} and "User B" write {string} at the row {int} column {int} at the same time', { timeout: STEP_TIMEOUT }, async function (userAText, userARow, userAColumn, userBText, userBRow, userBColumn) {
  try {
...
Then('"User B" should see the {string} at the row {int} column {int} and "User A" should see the {string} at the row {int} column {int}', { timeout: STEP_TIMEOUT }, async function (userAText, userARow, userAColumn, userBText, userBRow, userBColumn) {
