...
import { SELECTION } from 'constants/office_docx';
...
import { createTable, getTableCell } from 'utils/tableActions';
import { checkSingleMessageSynchronized } from 'utils/textEvaluation';
...
Given('"User A" and "User B" have opened the same document in same table', { timeout: STEP_TIMEOUT }, async function () {
...
When('"User A" creates column {int} row {int} a new table', { timeout: STEP_TIMEOUT }, async function (column, row) {
  withErrorHandling(() => createTable(pageA, column, row), "error occured while create table");
  tableColumnNum = column;
  tableRowNum = row;
})

When('"User A" selects first cell and write {string}', { timeout: STEP_TIMEOUT }, async function (text) {
...
When('"User B" selects second cell in the same table and write {string}', { timeout: STEP_TIMEOUT }, async function (text) {
...
Then('The table should be merged correctly with both {string} and {string}', async function (textA, textB) {
